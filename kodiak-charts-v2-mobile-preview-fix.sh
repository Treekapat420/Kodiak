...
ROUTE="$WEB/src/app/api/token/[mint]/chart/route.ts"
LAUNCH="$WEB/src/app/launch/page.tsx"

echo "🐻 Installing Kodiak Charts v2 and mobile preview fix..."

for file in "$CHART" "$ROUTE" "$LAUNCH"; do
  if [ ! -f "$file" ]; then
...
cp "$CHART" "$CHART.bak-charts-v2"
cp "$ROUTE" "$ROUTE.bak-charts-v2"
cp "$LAUNCH" "$LAUNCH.bak-mobile-preview"
...
route = Path('/workspaces/Kodiak/web/src/app/api/token/[mint]/chart/route.ts')
...
text = text.replace(
  'className="relative h-36 bg-gradient-to-br from-amber-300/20 via-zinc-900 to-emerald-400/10"',
  'className="relative h-20 sm:h-36 bg-gradient-to-br from-amber-300/10 via-zinc-950 to-emerald-400/5"',
  1,
)
text = text.replace('className="p-6"', 'className="p-4 sm:p-6"', 1)
text = text.replace(
...
text = text.replace(
  ...
  'className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-3xl sm:h-24 sm:w-24',
  1,
)
...
cd "$ROOT"
git add web/src/components/charts/LaunchChart.tsx web/src/app/api/token/[mint]/chart/route.ts web/src/app/launch/page.tsx
git commit -m "add Kodiak Charts v2 and fix mobile preview shading" || true
git push origin main

echo ""
echo "✅ Kodiak Charts v2 installed and pushed."
echo "After Vercel shows Ready, refresh TEST3 and the Launch page."
