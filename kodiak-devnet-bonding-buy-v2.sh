...
echo "🐻 Installing Kodiak Devnet bonding-buy v2..."
...
export default function TradePage() {
  ...
  const [status, setStatus] = useState<Status>({
    kind: "idle",
    message: "Load the last Kodiak launch or paste a Devnet mint.",
  });
  ...
  const loadLastLaunch = () => {
    ...
    try {
      ...
      if (!launch.mint) {
        throw new Error("The saved launch does not contain a mint address.");
      }
      ...
      setStatus({
        kind: "idle",
        message: "Saved Kodiak launch loaded. Now load its bonding curve.",
      });
    } catch (error) {
      setStatus({
        kind: "error",
  ...
  const buyToken = async () => {
    ...
    if (!mintIsValid) {
      setStatus({
        kind: "error",
        message: "Enter or load a valid Devnet mint first.",
      });
      return;
    }
    ...
    try {
      ...
      setStatus({
        kind: "working",
        message: "Simulating the buy before Phantom can sign…",
      });
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-xl text-zinc-400">
                Test a real Raydium LaunchLab purchase before adding
                selling.
              </p>
            </div>
            <KodiakWalletButton />
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <p className="text-sm text-zinc-400">
              Wallet: {publicKey ? walletSol ?? "Loading…" : "—"} SOL
            </p>
          </div>
          ...
          <p className="mt-4 text-xs leading-5 text-zinc-500">
            Slippage is fixed at 1% for this Devnet test.
          </p>
...
git commit -m "fix Kodiak Devnet bonding curve buy page" || true
git push origin main

echo ""
echo "✅ Kodiak Devnet bonding-buy v2 installed and pushed."
echo ""
echo "After Vercel shows Ready:"
echo "1. Open your REAL Kodiak URL in Phantom."
echo "2. Add /trade to the end."
echo "3. Tap Load Last Kodiak Launch."
echo "4. Tap Load Devnet Bonding Curve."
echo "5. Start with 0.001 Devnet SOL."
