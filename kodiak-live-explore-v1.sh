...
WEB="$ROOT/web"
EXPLORE="$WEB/src/app/explore/page.tsx"
API="$WEB/src/app/api/explore/route.ts"

echo "🐻 Installing live Kodiak Explore page..."

if [ ! -f "$EXPLORE" ]; then
  echo "Missing Explore page: $EXPLORE"
  exit 1
fi

mkdir -p "$(dirname "$API")"
cp "$EXPLORE" "$EXPLORE.bak-live-explore"
...
git commit -m "build live Redis-powered Explore page" || true
git push origin main

echo ""
echo "✅ Live Explore page installed and pushed."
echo ""
echo "After Vercel shows Ready, open /explore."
