...
ACTIVITY="$WEB/src/components/LiveActivity.tsx"

echo "🐻 Removing demo homepage data and switching Kodiak fully to live data..."
...
if [ ! -f "$ACTIVITY" ]; then
  echo "Missing LiveActivity component: $ACTIVITY"
  exit 1
fi

cp "$PAGE" "$PAGE.bak-remove-demo-data"
cp "$ACTIVITY" "$ACTIVITY.bak-remove-demo-data"
...
# Remove demo-only imports.
text = re.sub(
    r'^import\s+\{\s*LaunchGrid\s*\}\s+from\s+["\']@/components/LaunchGrid["\'];\s*\n?',
    '',
    text,
    flags=re.MULTILINE,
)
text = re.sub(
    r'^import\s+\{\s*Stats\s*\}\s+from\s+["\']@/components/Stats["\'];\s*\n?',
    '',
    text,
    flags=re.MULTILINE,
)

# Remove demo-only homepage sections.
text = re.sub(r'^\s*<Stats\s*/>\s*\n?', '', text, flags=re.MULTILINE)
text = re.sub(r'^\s*<LaunchGrid\s*/>\s*\n?', '', text, flags=re.MULTILINE)
...
git add web/src/app/page.tsx web/src/components/LiveActivity.tsx
git commit -m "remove demo homepage stats and launch cards" || true
git push origin main

echo ""
echo "✅ Demo homepage data removed."
echo "✅ Homepage market sections now use only live Kodiak data."
echo "✅ Changes pushed to main."
echo ""
echo "After Vercel is Ready, refresh the normal Kodiak homepage."
