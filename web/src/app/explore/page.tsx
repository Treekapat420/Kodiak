    ...
    const load =
      async () => {
        try {
          ...
          const payload =
            (await response.json()) as ExplorePayload;
          ...
          if (
            ...
            throw new Error(
              `Explore returned ${payload.network} data while Kodiak is configured for ${KODIAK_NETWORK}.`,
            );
          }
          ...
          setMessage(
            next.length
              ? `${next.length} live Kodiak launch${next.length === 1 ? "" : "es"} on ${NETWORK_LABEL}`
              : `No ${NETWORK_LABEL} launches have been registered yet.`,
          );
        } catch (error) {
          if (
            !cancelled
